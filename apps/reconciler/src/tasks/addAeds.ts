...
export const addAeds = async ({
  ...
  for (const registerAed of registerAedsById.values()) {
    ...
    console.log(
      `${reconcilerConfig.dryRun ? "[dry] Would create" : "Planned create"} node for register AED ${registerAed.ASSET_GUID} at ${registerAed.SITE_LATITUDE},${registerAed.SITE_LONGITUDE}`,
    );
