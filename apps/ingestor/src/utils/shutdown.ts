...
export const installShutdownHandlers = ({
  ...
  const shutdown = async (signal: NodeJS.Signals): Promise<void> => {
    ...
    // Drain workers first so they stop picking up new jobs and active
    // jobs receive an abort signal once the per-worker timeout elapses.
    await Promise.all(
    ...
    // Close queues so no further jobs/scheduler updates are pushed.
    await Promise.all(
