CREATE TABLE "osm_planet_import_state" (
	"source_url" text PRIMARY KEY NOT NULL,
	"file_path" text NOT NULL,
	"remote_etag" text,
	"remote_last_modified" timestamp with time zone,
...
CREATE TABLE "osm_replication_state" (
	...
	"created_at" timestamp with time zone DEFAULT now() NOT NULL,
	"updated_at" timestamp with time zone DEFAULT now() NOT NULL,
	CONSTRAINT "osm_replication_state_sequence_number_check" CHECK ("osm_replication_state"."sequence_number" >= 0)
);
--> statement-breakpoint
...
CREATE TABLE "osm_aed_history" (
	...
	CONSTRAINT "osm_aed_history_latitude_check" CHECK ("osm_aed_history"."latitude" IS NULL OR "osm_aed_history"."latitude" BETWEEN -90 AND 90),
	CONSTRAINT "osm_aed_history_longitude_check" CHECK ("osm_aed_history"."longitude" IS NULL OR "osm_aed_history"."longitude" BETWEEN -180 AND 180),
	CONSTRAINT "osm_aed_history_source_check" CHECK ("osm_aed_history"."source" IN ('planet', 'minute')),
	CONSTRAINT "osm_aed_history_replication_sequence_check" CHECK ("osm_aed_history"."replication_sequence" IS NULL OR "osm_aed_history"."replication_sequence" >= 0)
);
--> statement-breakpoint
CREATE INDEX "osm_aed_deleted_at_idx" ON "osm_aed" USING btree ("deletedAt");--> statement-breakpoint
